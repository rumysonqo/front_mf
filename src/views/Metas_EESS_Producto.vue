...
async function get_establecimiento(){
  try {
      const response = await axios.get(url+'establecimiento/'+cod_mic)
      establec.value = response.data
    } catch (e) {
      error.value = e
    }
}
...
async function get_metas_MR_EESS_Producto(){
  try {
      const response = await axios.get(url+'metas_MR_EESS_Producto/'+cod_prg+'/'+cod_prd+'/'+cod_est)
    ...
    <div class="container-fluid">
      <div class="row">
        ...
        <div>
            ...
            <select v-model="cod_mic" @change="get_establecimiento()" class="form-select form-select-md mb-3">
                <option value="" disabled selected>Seleccione una Micro Red</option>
                <option v-for="mic in micro_red" :value="mic.cod_micro_red">
                  {{ mic.nom_micro_red }}
                </option>
              </select>
              <select v-model="cod_est" @change="get_metas_MR_EESS_Producto()" class="form-select form-select-md mb-3">
                <option value="" disabled selected>Seleccione un establecimiento</option>
                <option v-for="est in establec" :value="est.cod_establecimiento">
                  {{ est.nom_establecimiento }}
                </option>
              </select>
      ...
      <div class="col-md-6">
        <div>
          ...
          <select v-model="cod_prd" @change="get_metas_MR_EESS_Producto()" class="form-select form-select-md mb-3">
      ...
      <DataTable :data="metas" :columns="columns" :options="opciones" class=" hover cell-border table compact table-striped display">
        <thead>
          <tr>
              ...
              <th style=" width: 30%; text-align: center;">Sub Producto</th>
              <th style=" width: 5%; text-align: center;">Trazador</th>
              <th style=" width: 20%; text-align: center;">Unidad de Medida</th>
